    ...
    with torch.no_grad():
        ...
        for i in range(N_samples):
            save_video(rgb[i], os.path.join(eval_dir,
                                            f'generated_{num_vid}_rgb.mp4'),
                       as_gif=False, fps=fps)
            # xray_recons_image = convert_torch_float_to_numpy_image(rgb[i][0], 'xray_recons.png')
            # depth_recons = convert_torch_float_to_numpy_image(depth[i], 'depth_recons.png')
            break

def convert_torch_float_to_numpy_image(t, save = ''):
    #Detach CPU
    t_array = t.detach().cpu().numpy()

    # Normalize the values from -1 to 1 to 0 to 1
    t_array = (t_array + 1) / 2

    # Scale from 0 to 1 to 0 to 255 and convert to unsigned 8-bit integer
    t_array = (t_array * 255).astype(np.uint8)
    print('Tarray shape:', t_array.shape)
    # print('Test xray:', t_array[0][:2])

    # t_array = t_array.squeeze(0)  # Now the shape is (3, 64, 64)
    # print('Target Xray shape after squeeze:', t_array.shape)

    # Transpose the array to shape (64, 64, 3) to match the expected HWC format for images
    t_array = np.transpose(t_array, (1, 2, 0))
    # print('Target Xray shape after transpose:', t_array.shape)
    # print('Min Target XRay value:', np.min(t_array))
    # print('Max Target XRay value:', np.max(t_array))
    if len(save) > 0:
        imageio.imwrite(save, t_array)
    return t_array
...
def reconstruct(args, config_file):
    ...
    _, hwfr, test_render = get_data(config_file)
    config_file['data']['hwfr'] = hwfr
    # print('\nData length: ', len(hwfr))
    # print('First Data:', hwfr[:5])
    # print('Test render shape', test_render.shape)
    # print('Test render', test_render[0])

    # plt.imsave('Image_render.png', test_render)
    ...
    print('\nLoaded %s' % os.path.join(checkpoint_dir, model_file))
    ...
    # print('Target Xray Path: ', target_xray)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_every", default=5000, type=int, help="save video of projections every number of iterations")
    parser.add_argument("--psnr_stop", default=30.1, type=float, help="stop at this psnr value")
    ...
    out_dir = config_file['training']['outdir']
    checkpoint_dir = os.path.join(out_dir, 'chkpts')
    os.makedirs(os.path.join(out_dir, args.save_dir), exist_ok=True)
    num_vid = '01'
    eval_dir = os.path.join(os.path.join(out_dir, args.save_dir), num_vid) # change parameter 01 here
