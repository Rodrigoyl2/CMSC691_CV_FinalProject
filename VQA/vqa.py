...
if __name__ == "__main__":
    ...
    for i in range(1, 29):
        # video_reader = decord.VideoReader(vid_path + '/' + optional_mess + f'generated_{item}_rgb.mp4')
        video_reader = decord.VideoReader(vid_path + '/' + f'generated_{optional_mess}_{str(i)}_vid.mp4')
    ...
    print(f"The quality score of all videos is {[f'{sc:.5f}' for sc in score_lis]}.")

    with open(os.path.join(vid_path, 'vqa_scores.txt'), 'w') as file:
        for sc in score_lis:
            # Format the score to suppress scientific notation and write to file
            formatted_score = f"{sc:.5f}"
            file.write(f"{formatted_score}\n")
